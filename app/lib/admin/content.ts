...
export async function listContentFiles(
  ...
  if (dbEntries.length > 0) {
    dbEntries.forEach((entry) => {
      ...
      if (entry.path.startsWith('menu/') && entry.path.endsWith('.json')) {
        const slug = entry.path.replace('menu/', '').replace('.json', '');
        addItem({
          id: `menu-${slug}`,
          label: `Menu: ${titleCase(slug)}`,
          path: entry.path,
          scope: 'locale',
        });
        return;
  ...
  const menuDir = path.join(CONTENT_DIR, siteId, locale, 'menu');
  try {
